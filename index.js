...
function renderGoalInputPage(container) {
    const pageDiv = document.createElement('div');
    ...
    restrictions.forEach(r => {
        restrictionsGroup.appendChild(createCheckbox(r, 'dietaryRestrictions', userGoals.dietaryRestrictions.includes(r), (checked) => {
            if (checked)
                userGoals.dietaryRestrictions.push(r);
            else
                userGoals.dietaryRestrictions = userGoals.dietaryRestrictions.filter(item => item !== r);
        }));
    });
    ...
    goals.forEach(g => {
        primaryGoalGroup.appendChild(createRadio('primaryGoal', g.label, g.value, userGoals.primaryGoal === g.value, (value) => userGoals.primaryGoal = value));
    });
...
async function handleSubmitGoals() {
    ...
    try {
        ...
        if (!userGoals.mealsConsumed.breakfast)
            parsedPlan.breakfast = null;
        if (!userGoals.mealsConsumed.lunch)
            parsedPlan.lunch = null;
        if (!userGoals.mealsConsumed.dinner)
            parsedPlan.dinner = null;
        if (!userGoals.mealsConsumed.snacks)
            parsedPlan.snacks = null;
        mealPlan = parsedPlan;
...
function renderMealPlanPage(container) {
    const pageDiv = document.createElement('div');
    pageDiv.className = 'page';
    ...
    const mealsToShow = ['breakfast', 'lunch', 'dinner', 'snacks'];
    let planIsEmpty = true;
    mealsToShow.forEach(mealKey => {
        if (userGoals.mealsConsumed[mealKey]) {
            const mealData = mealPlan[mealKey];
...
// --- Helper Functions ---
function createFormGroup(labelText, sectionIdSuffix) {
...
function createCheckbox(labelText, name, checked, onChange) {
...
function createTextArea(name, value, onInput, labelIdToAssociate) {
